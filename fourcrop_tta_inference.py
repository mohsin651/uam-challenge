"""4-crop TTA inference on the 0.15884 8-ckpt ensemble.

For each input image (already at 256×128 from the loader):
  1. Upsample to 280×140 (10% larger)
  2. Take 4 corner crops, each 256×128 (no pos_embed change — avoids the
     manifold-drift that killed multi-scale TTA in §71)
  3. Run model on each crop, L2-normalize, average to get per-ckpt feature
Then the usual 8-ckpt ensemble + DBA(k=8) + rerank(k1=15, k2=4, λ=0.275)
+ class-group filter.

Generates ONE primary CSV. Per the 1-2 high-EV variant rule.
"""
import csv
import os
import numpy as np
...
def extract_4crop(model, loader, num_query):
    """Extract features with 4-corner-crop TTA (input upsampled to 280x140 then 4x256x128 crops)."""
    feats = []
    with torch.no_grad():
        for data in loader:
            img = data['images'].cuda()  # (B, 3, 256, 128)
            # Upsample 256×128 → 280×140 (10% larger), then 4 corner crops
            img_up = F.interpolate(img, size=(280, 140), mode='bilinear', align_corners=False)
            crops = [
                img_up[:, :, 0:256, 0:128],     # top-left
                img_up[:, :, 0:256, 12:140],    # top-right
                img_up[:, :, 24:280, 0:128],    # bottom-left
                img_up[:, :, 24:280, 12:140],   # bottom-right
            ]
            # Forward pass per crop, L2-normalize, average
            crop_feats = []
            for c in crops:
                f = model(c).float()
                f = F.normalize(f, p=2, dim=1)
                crop_feats.append(f)
            ff = torch.stack(crop_feats, dim=0).mean(dim=0)  # avg across 4 crops
            ff = F.normalize(ff, p=2, dim=1)                  # re-normalize
            feats.append(ff.cpu())
...
def main():
    ...
    for ckpt in ALL_CKPTS:
        print(f"  4-crop extraction: {os.path.basename(os.path.dirname(ckpt))}/{os.path.basename(ckpt)}")
        model = make_model(cfg, cfg.MODEL.NAME, 0, 0, 0)
        model.load_param(ckpt)
        model = model.cuda().eval()
        qf, gf = extract_4crop(model, val_loader, num_query)
    ...
    out = '/workspace/miuam_challenge_diff/results/fourcrop_tta_baseline8_submission.csv'
