...
# deep supervision on last N intermediate transformer layers (path B)
_C.MODEL.DEEP_SUP = False
_C.MODEL.NUM_AUX_LAYERS = 5
_C.MODEL.AUX_LOSS_WEIGHT = 0.1

# warm-start fine-tune: if non-empty, load weights from this checkpoint after
# the standard ImageNet init. Used for heavy-aug fine-tunes that start from
# an already-trained ReID model rather than ImageNet.
_C.MODEL.FINETUNE_FROM = ''

# Circle Loss hyperparameters (used when MODEL.METRIC_LOSS_TYPE contains 'circle')
...
# ---------------------------------------------------------------------------- #
_C.SOLVER = CN()
# Gradient clipping max L2 norm (0.0 = disabled). Safety net against AMP missing
# NaN — added after DINOv2 run blew up at ep40 despite loss-scaler.
_C.SOLVER.GRAD_CLIP = 0.0
# Exponential Moving Average of model weights (0.0 = disabled). Typical values:
# 0.9999 (slow, whole-training avg) or 0.999 (faster). Applied at save-time.
