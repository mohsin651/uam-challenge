"""Light-aug cam-adv ep60 — diagnostic inference (2 variants).

Mirrors the heavyaug experiment but tests whether ColorJitter+RandomErasing+LGT
alone (without spatial ops Perspective/Rotation) keep features ensemble-compatible.

Generates 2 CSVs:
  REPLACE: 7-baseline + lightaug ep60 (no original cam-adv) — direct diagnostic
  STACK:   8-baseline + lightaug ep60 (proven 0.15884 + lightaug as add-on)
"""
import csv
import os
import numpy as np
...
LIGHTAUG_EP60 = '/workspace/miuam_challenge_diff/models/model_vitlarge_camadv_lightaug_seed1300/part_attention_vit_60.pth'
...
def main():
    ...
    print("\n  Extracting 7-baseline ensemble:")
    ...
    model = model.cuda().eval()
    s500_qf, s500_gf = extract(model, val_loader, num_query)
    del model; torch.cuda.empty_cache()

    print(f"\n  lightaug s1300 ep60:")
    model = make_model(cfg, cfg.MODEL.NAME, 0, 0, 0)
    model.load_param(LIGHTAUG_EP60)
    model = model.cuda().eval()
    light_qf, light_gf = extract(model, val_loader, num_query)
    del model; torch.cuda.empty_cache()

    os.makedirs(OUT_DIR, exist_ok=True)
    # REPLACE variant: 7-baseline + lightaug (no s500)
    write_csv(base_qf + light_qf, base_gf + light_gf,
              'lightaug_replace_baseline7_plus_ep60', num_query, val_loader)
    # STACK variant: 8-baseline + lightaug (s500 + lightaug)
    write_csv(base_qf + s500_qf + light_qf, base_gf + s500_gf + light_gf,
              'lightaug_stack_baseline8_plus_ep60', num_query, val_loader)
