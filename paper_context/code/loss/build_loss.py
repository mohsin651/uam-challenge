...
def build_loss(cfg, num_classes):
    ...
    if 'circle' in cfg.MODEL.METRIC_LOSS_TYPE:
        # Circle Loss as drop-in replacement for triplet. Typically +1-3% mAP.
        triplet = CircleLoss(margin=cfg.MODEL.CIRCLE_MARGIN, gamma=cfg.MODEL.CIRCLE_GAMMA)
        # wrap so calling triplet(feat, target) returns a (loss, _) tuple like TripletLoss
        _circle = triplet
        class _CircleWrapper:
            def __call__(self, feat, target):
                return (_circle(feat, target),)
        triplet = _CircleWrapper()
        print(f"using CircleLoss (margin={cfg.MODEL.CIRCLE_MARGIN}, gamma={cfg.MODEL.CIRCLE_GAMMA})")
        ...
        print('expected METRIC_LOSS_TYPE should be triplet or circle'
    ...
    # softmax & triplet
        ...
        def loss_func(score, feat, target, domains=None, t_domains=None, all_posvid=None, soft_label=False, soft_weight=0.1, soft_lambda=0.2):
            # Circle Loss uses the same loss_func path as triplet — it's wrapped so
            # triplet(feat, target)[0] works identically.
            if cfg.MODEL.METRIC_LOSS_TYPE in ('triplet', 'circle'):
                if cfg.MODEL.IF_LABELSMOOTH == 'on':
                    if name == 'local_attention_vit' and cfg.MODEL.PC_LOSS:
